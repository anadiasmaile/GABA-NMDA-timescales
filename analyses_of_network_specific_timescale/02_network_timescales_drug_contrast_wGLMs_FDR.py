...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 25 13:20:56 2025

@author: okohl

Calculate Lora vs Placebo contrast for Network-specific timescales.
Withins GLMs testing whether partcicipants' Lorazepam - Placebo difference is significantly
different from 0. Obtained p-values were fdr corrected across states and parcels.

Plot percentage of change of state-specific timescales due to Lora intake 
calculate as percenage of change from participants placebo timescale.
Surface plots only show percentage of change of parcels that show significant differences
between Placebo und Loracepam.
...
# Set dirs
output_dir = '.../results/hmm/08_states/fooof/per_vertex_hmm/time_constant_contrasts_percentage/lateral'
...
# Lora
tc_lora = tc[tc['drug']=='lora']
tc_lora = pivot_df(tc_lora)
tc_lora = tc_lora[:,reindexer]
...
# Calculate 1 Sample Ttest for all parcels and states
diff = tc_lora - tc_plac
t, p, dof, _ , _ = condition_contrast_glms(diff,pooled_dims=[])
  
# FDR Correction 
lora_fdr_corrected_pmask, lora_fdr_corrected_pvalues = fdr_correction(p)


#%% Plotting

# Lora vs Plac - FDR Change

 for iK in range(8):
    
     change = (tc_lora[:, :, iK] - tc_plac[:,:,iK]) / tc_plac[:,:,iK] * 100 # percentage of change for time scales
     maps = np.median(change, axis=0)
     maps[~lora_fdr_corrected_pmask[:,iK]] = 0 # set non-significant parcels to 0
     ...
     power.save(
         ...
         filename=f"{output_dir}/state{iK}_lora_plac_contrast_fdr_thresh_percentage_of_change_.svg",
     )
    
     plt.close()
    

#%% Make large overview plot showing percentage of change across all networks
all_maps = []
for iK in range(8):           
    change = (tc_lora[:, :, iK] - tc_plac[:,:,iK]) / tc_plac[:,:,iK] * 100 # percentage of change
    maps = np.median(change, axis=0)
    maps[~lora_fdr_corrected_pmask[:,iK]] = 0 # Set non-significant parcels to 0
        
    all_maps.append(maps)
   
# Get value defining common range for all states
...
power.save(
    ...
    combined=True,
    n_rows=2,
    filename=f"{output_dir}/combined_lora_plac_contrast_fdr_thresh_percentage_of_change_wcbar_.png",
)

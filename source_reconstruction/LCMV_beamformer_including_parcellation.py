...
save_src = True
# apply orthogonalisation (leakage)
...
""" PARCELLATE SOURCE ESTIMATE INTO DESIKAN ATALS """
log.info(f"Apply cortical parcellation based on Desikan atlas")
labels = mne.read_labels_from_annot(f'sub-{sub:02d}', parc="aparc",
                                    subjects_dir=subjects_dir, hemi = "both")
parcel_names = [label.name for label in labels]
parcel_ts = mne.extract_label_time_course(stc, labels, src, mode = "pca_flip")
 plot correlations before leakage
parcellation.plot_correlation(parcel_ts, filename=op.join(qc_dir, ep_stub % 'pre-correlation_parcels_psd.png'))

""" LEAKAGE CORRECTION """
# this function reduces the signal leakage by regressing out zero lag
# correlations between parcel timce courses. This step is quite agressive but
# necessary when looking at coherence/running HMMs.
if orthogonalisation == True:
    log.info(f"Appling symmetric leakage correction on parcellated data.")
    parcel_ts = parcellation.symmetric_orthogonalise(parcel_ts, maintain_magnitudes=True)

""" WRITE DATA TO RAW FILE AGAIN - SO THAT WE CAN USE MNE FUNCTIONS """
log.info(f"Writing parcellated data to raw.")
# use OSL function to write npy to raw
parc_raw = parcellation.convert2mne_raw(parcel_ts, raw, parcel_names=parcel_names,
                                        extra_chans="stim")
# save parcellation
parc_raw.save(op.join(deriv_dir,ep_stub % 'parc_raw.fif'), overwrite=True)
